...
THREE.ARControls = function ( camera, speed, done ) {
	...
	this._init = function () {
		...
		if ( navigator.getVRDevices ) {
			navigator.getVRDevices().then( gotARDevices );
		} else {
			navigator.mozGetVRDevices( gotARDevices );
		}

		function gotARDevices( devices ) {
			var arInput;
			var error;
			for ( var i = 0; i < devices.length; ++i ) {
				if ( devices[i] instanceof PositionSensorVRDevice ) {
					arInput = devices[i]
					self._arInput = arInput;
	...
	this.update = function() {
      ...
      var state = arState.hmd.rotation;
      if (arState.hmd.rotation[0] !== 0 ||
					arState.hmd.rotation[1] !== 0 ||
					arState.hmd.rotation[2] !== 0 ||
					arState.hmd.rotation[3] !== 0) {
					var arStateRotation = new THREE.Quaternion(state[0], state[1], state[2], state[3]);
	        totalRotation.multiplyQuaternions(manualRotation, arStateRotation);
      } else {
	...
	this.getRotation = function() {
		if ( typeof arState == "undefined" || !arState ) {
	...
	this.getARState = function() {
		var arInput = this._arInput;
		var orientation;
		var arState;

		if ( arInput ) {
			orientation	= arInput.getState().orientation;
